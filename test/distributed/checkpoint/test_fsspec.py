...
class TestFSSpec(ShardedTensorTestBase):
    ...
    def test_fsspec(self, device):
        ...
        model = FSDP(MyTestModule().to(torch.device(device)))
        ...
        model_2 = FSDP(MyTestModule().to(torch.device(device)))
    ...
    @with_comms(init_rpc=False, backend="hccl" if TEST_HPU else "nccl")
    @skip_if_lt_x_gpu(2)
    # @requires_nccl()
    @with_temp_dir
...
instantiate_device_type_tests(TestFSSpec, globals(), only_for=devices)
